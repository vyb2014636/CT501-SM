...
const NewUsers = () => {
  ...
  const fetchTotalUser = async () => {
    ...
    try {
      const totalUserResponse = await getStatisticsDetailsAPI('users')

      // Lấy dữ liệu từ response
      setTotalUsers(totalUserResponse.stats.usersThisMonth || [])

      // Lọc các tháng có dữ liệu từ createdAt
      const monthsWithData = totalUserResponse.stats.allUsers.reduce((months, user) => {
  ...
  const fetchLoadTotalUser = async (selectedMonth, selectedYear) => {
    ...
    try {
      const totalUserResponse = await getStatisticsDetailsAPI('users', selectedMonth, selectedYear)
      setTotalUsers(totalUserResponse.stats.usersThisMonth || [])
              ...
              <Grid item xs={12}>
                <Card sx={{ padding: 3, boxShadow: 3 }}>
                  <Typography variant='h6' color='textSecondary' align='center'>
                    Không tìm thấy người dùng trong tháng
                  </Typography>
                </Card>
              </Grid>
            )}
